 ...
 */
export const isFileOutdated = async (outputFilePath: string, config: Config) => {
  const outputLastWrittenTimeStamp = await getLastModifiedTime(outputFilePath);
 ...
 */
const scheduleCleanup = (imageFilePath: string, timestamp: number) => {
  ...
  setTimeout(timeoutCallback, cleanupDelay);
};
 ...
 * timestamp.
 */
const removeFilesOlderThan = async (directoryPath: string, timestamp: number) => {
  let fileNames;
  try {
